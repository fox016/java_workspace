...
public class AddItemBatchController extends Controller implements
	...
	public void useScannerChanged() {
		...
		if(useScanner&&!oldUseScanner)
		{
			loadValues();
		}
		enableComponents();
                oldUseScanner = useScanner;
	}
    ...
    private List<Item> addBatchItems(Product product, Calendar cal, StorageUnit root, int count) 
    {
        ...
        UnitDataObject unitObj = PersistenceFactory.createUnitDataObject(root);
        pf.updateUnit(unitObj);
		pf.end();
	//EDIT by Chris
		  /*
		  try{
			  Facade.addBatchItems(toReturn);
			}
			catch(Exception e) {
				e.printStackTrace();
			}
    ...
    private void handleViewRefreshUndo(Command c) 
    {
        ...
        allBatchItems.get(pBarcode).removeAll(addedItems);
        if (allBatchItems.get(pBarcode).isEmpty())
        {
            allBatchItems.remove(pBarcode);
