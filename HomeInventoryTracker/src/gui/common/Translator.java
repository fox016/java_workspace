 ...
 * @author Talonos
 */
public class Translator  //Why is this not static! Augh!
{
	public Translator()
	{	
	}
    ...
    public ItemData getDataFromItem(Item i) throws IllegalArgumentException
    {
    	...
    	ProductContainer container =
    			Facade.getProductContainer(i.getStorageUnit(), i.getProduct());
        if(container != null)
        {
        	if(container instanceof ProductGroup)
        	{
        		toReturn.setProductGroup(container.getName());
        	}
        	else
    ...
    public ProductData getDataFromProduct(Product p) throws IllegalArgumentException
    {
        ...
        toReturn.setSupply(new Size(p.getThreeMonthSupply(), SupplyType.COUNT).toString());
        toReturn.setTag(p);
        return toReturn;
    }
	...
	/**
	 * Converts a set of ProductBarcodes into an array of ProductData. This array
         * will be sorted by description, not by barcodes, as of 2/26/2013. I hope
         * that by doing this I fixed more than I broke.
	 * 
	 * @param barcodes
	 * @return
	 */
	public ProductData[] getDataFromProductBarcodes(Set<ProductBarcode> barcodes) {
		...
		if(barcodes == null)
			return null;
                ...
                for (ProductBarcode tCode : barcodes)
                {
                    Product nextProduct = ProductManager.instance().getProductByBarcode(tCode);
                    dataTree.add(this.getDataFromProduct(nextProduct));
                }
