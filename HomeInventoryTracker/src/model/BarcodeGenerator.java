...
public class BarcodeGenerator implements Serializable {
	...
	public static final int BARCODE_SIZE = 12;
	...
	public void setLastCode(int[] lc)
	{
		if(lc.length != BARCODE_SIZE)
			throw new IllegalArgumentException("Attempting to set a wierd lastCode");
		lastCode = lc;
	}

	public void setLastID(int newId)
	{
		lastProductContainerCode = newId;
	}
	...
	/**
	 * @return Last generated barcode string
	 */
	public String getLastBarcode() {
		...
		char[] charCode = new char[BARCODE_SIZE];
		for(int i = 0; i < charCode.length; i++)
			charCode[i] = (Integer.toString(lastCode[i])).charAt(0);
		
		return new String(charCode);
	}

	/**
	 * @return Last generaged product container id
	 */
